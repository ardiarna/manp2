...
function doOnLoad() {
    ...
    grid_reff.setHeader("No.,Asset#,Description,MTTR,Location,Sub Location,Group,Category,Status");
	...
	grid_reff.init();
	grid_reff.load(FRM+"?mode=view&location="+myFormSearch.getItemValue('location')+"&sublocation="+myFormSearch.getItemValue('sublocation')+"&from_date="+myFormSearch.getItemValue('from_date', true)+"&to_date="+myFormSearch.getItemValue('to_date', true), "xml");	
...
function openRequestDetails(windows, options) {
	...
	const winTitle = `Mean Time To Repairs - ${options.id}`;
	win.centerOnScreen();
	win.setText(winTitle);
	win.button("park").hide();
	win.setModal(true);
	win.maximize();
	win.attachURL("mttrmtbf_dtl.php?asset="+options.id);
