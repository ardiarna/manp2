  ...
  <script>
	...
	function doOnLoad() {
		const rootLayout = new dhtmlXLayoutObject({
			...
			pattern: '2E',
			cells: [
				{id: "a", text: "Work Order List", header: true, height:80},
...
*/
        ...
        grid_reff.setHeader("No.,WO#,Date,Status,Description,Due Date,Duration Est.,Duration Real,Scheduled Est.,Start Scheduled Real,End Scheduled Real,Approve By,Complete By,Compl. Date", null,[TC,TL,TC,TL,TL,TC,TL,TL,TC,TC,TC,TL,TL,TC]);
	...
	function openRequestDetails(windows, options) {
		const win = windows.createWindow('dwg_request_details', 0, 0, 600, 530);
		const winTitle = `Complete Work Order : ${options.id}`;
		const DURATIONSAT_OPTIONS = {'Hours': 'Hours', 'Minutes': 'Minutes'};
		...
		const buttons = [
      	...
      	var forminput = [
			...
			{type: "block",width: 530,blockOffset: 20,list:[
			...
			{type: "block", inputWidth: "auto", id: "form_cell_a", list:[	
				...
				{type: "block",width: 400,blockOffset: 0, offsetTop:20, list:[
				...
				{type:"calendar", name: "realschstart", button: "calendar_icon",readonly:true, label:"Start Scheduled Date Real : ", calendarPosition: "right",inputWidth: 150, dateFormat: "%Y-%m-%d %H:%i:%s", required: true},
				{type:"calendar", name: "realschend", button: "calendar_icon",readonly:true, label:"End Scheduled Date Real : ", calendarPosition: "right",inputWidth: 150, dateFormat: "%Y-%m-%d %H:%i:%s", required: true}
			]},
