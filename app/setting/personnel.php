...
*/
		toolbar.attachEvent('onClick', itemId => {
			...
			if(itemId === 'baru') {
			  ...
			  if (!selectedOrderId) {
				...
				const selectedOrderId = grid_reff.getSelectedRowId();
			  	if (!selectedOrderId) {
			  		alert('Anda belum memilih personnel yang ingin dihapus');
					return;
			  	}
				const desc = grid_reff.cells(selectedOrderId, grid_reff.getColIndexById('se_name')).getValue();
				dhtmlx.confirm({
					title: 'Delete Personnel',
					type: 'confirm-error',
					text: 'Apakah Anda yakin ingin menghapus personnel dengan kode '+selectedOrderId+' [Desc: '+desc+'] ?',
					callback: confirmed => {
						if (confirmed) {
							dhx.ajax.post(FRM+"?mode=delete", "kd="+selectedOrderId, function(resp){
	    						if(resp.xmlDoc.responseText == "OK"){
									dhtmlx.alert({
										title: "Info Hapus Personnel",
        ...
        grid_reff.setHeader("No.,Personnel Code,Description,Department,Position", null,
        ...
        grid_reff.setColumnIds('no,se_code,se_name,deparment,posisi');		
        grid_reff.setColTypes('ron,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt');
        grid_reff.setInitWidths("50,150,*,200,200");
		...
		grid_reff.attachHeader(",#text_filter,#text_filter,#select_filter,#select_filter");
...
function openRequestDetails(windows, options) {
		  ...
		  const winTitle = ['ubah', 'view'].includes(MODE) ? `Personnel - ${options.id}` : 'New Personnel';
		...
		{type: "block", offsetTop:0,offsetLeft:20,blockOffset: 0,list:[
			...
			{type: 'input', name: 'kodesat', label: 'Personnel Code : ', maxLength: 8,inputWidth: 100,required: true,maxlength:3},
			{type: 'input', name: 'nama', label: 'Name : ', inputWidth: 330},
			{type:"combo", name: "deparment", label:"Department : ",inputWidth: 150,required: true,options: [
							{text: "",value:""}, 
			...
			{type:"combo", name: "position", label:"Position : ",inputWidth: 150,required: true,options: [
							{text: "Operator",value:"O"}, 
							{text: "Staff",value:"S"},
							{text: "Kasubsi",value:"K"},
							{text: "Kabag",value:"B"},
