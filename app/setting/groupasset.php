...
function doOnLoad() {
		...
		const rootLayout = new dhtmlXLayoutObject({
			  ...
			  pattern: '1C',
			  cells: [{id: "a",header: false,text: 'Group Assets'}],
...
*/
		toolbar.attachEvent('onClick', itemId => {
			...
			if(itemId === 'baru') {
			  ...
			  if (!selectedOrderId) {
				...
				const selectedOrderId = grid_reff.getSelectedRowId();
			  	if (!selectedOrderId) {
			  		alert('Anda belum memilih assets group yang ingin dihapus');
					return;
			  	}
				const desc = grid_reff.cells(selectedOrderId, grid_reff.getColIndexById('sag_desc')).getValue();
				dhtmlx.confirm({
					title: 'Delete Assets Group',
					type: 'confirm-error',
					text: 'Apakah Anda yakin ingin menghapus assets group dengan kode '+selectedOrderId+' [Desc: '+desc+'] ?',
					callback: confirmed => {
						if (confirmed) {
							dhx.ajax.post(FRM+"?mode=delete", "kd="+selectedOrderId, function(resp){
	    						if(resp.xmlDoc.responseText == "OK"){
									dhtmlx.alert({
										title: "Info Hapus Assets Group",
        ...
        grid_reff.setHeader("No.,Group Code,Description", null,
        ...
        grid_reff.setColumnIds('no,sag_code,sag_desc');		
        grid_reff.setColTypes('ron,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt');
        grid_reff.setInitWidths("50,100,*");
...
function openRequestDetails(windows, options) {
		  ...
		  const winTitle = ['ubah', 'view'].includes(MODE) ? `Assets Group - ${options.id}` : 'New Asset Group';
		...
		{type: "block", offsetTop:0,offsetLeft:20,blockOffset: 0,list:[
			...
			{type: 'input', name: 'kodesat', label: 'Group Code : ', maxLength: 2,inputWidth: 100,required: true,maxlength:3},
