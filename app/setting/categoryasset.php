...
function doOnLoad() {
		...
		const rootLayout = new dhtmlXLayoutObject({
			  ...
			  pattern: '1C',
			  cells: [{id: "a",header: false,text: 'Category Assets'}],
...
*/
		toolbar.attachEvent('onClick', itemId => {
			...
			if(itemId === 'baru') {
			  ...
			  if (!selectedOrderId) {
				...
				const selectedOrderId = grid_reff.getSelectedRowId();
			  	if (!selectedOrderId) {
			  		alert('Anda belum memilih assets category yang ingin dihapus');
					return;
			  	}
				const desc = grid_reff.cells(selectedOrderId, grid_reff.getColIndexById('sac_desc')).getValue();
				dhtmlx.confirm({
					title: 'Delete Assets Category',
					type: 'confirm-error',
					text: 'Apakah Anda yakin ingin menghapus assets category dengan kode '+selectedOrderId+' [Desc: '+desc+'] ?',
					callback: confirmed => {
						if (confirmed) {
							dhx.ajax.post(FRM+"?mode=delete", "kd="+selectedOrderId, function(resp){
	    						if(resp.xmlDoc.responseText == "OK"){
									dhtmlx.alert({
										title: "Info Hapus Assets Category",
        ...
        grid_reff.setHeader("No.,Category Code,Description", null,
        ...
        grid_reff.setColumnIds('no,sac_code,sac_desc');		
...
function openRequestDetails(windows, options) {
		  ...
		  const winTitle = ['ubah', 'view'].includes(MODE) ? `Assets Category - ${options.id}` : 'New Asset Category';
