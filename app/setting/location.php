...
function doOnLoad() {
		...
		const rootLayout = new dhtmlXLayoutObject({
			  ...
			  pattern: '1C',
			  cells: [{id: "a",header: false,text: 'Jenis Barang'}],
...
*/
		toolbar.attachEvent('onClick', itemId => {
			...
			if(itemId === 'baru') {
			  ...
			  if (!selectedOrderId) {
				...
				const selectedOrderId = grid_reff.getSelectedRowId();
			  	if (!selectedOrderId) {
			  		alert('Anda belum memilih location yang ingin dihapus');
					return;
			  	}
				const desc = grid_reff.cells(selectedOrderId, grid_reff.getColIndexById('sl_desc')).getValue();
				dhtmlx.confirm({
					title: 'Delete Location',
					type: 'confirm-error',
					text: 'Apakah Anda yakin ingin menghapus location dengan kode '+selectedOrderId+' [Desc: '+desc+'] ?',
					callback: confirmed => {
						if (confirmed) {
							dhx.ajax.post(FRM+"?mode=delete", "kd="+selectedOrderId, function(resp){
	    						if(resp.xmlDoc.responseText == "OK"){
									dhtmlx.alert({
										title: "Info Hapus Location",
        ...
        grid_reff.setHeader("No.,Code,Description", null,
        ...
        grid_reff.setColumnIds('no,sl_code,sl_desc');		
...
function openRequestDetails(windows, options) {
		  ...
		  const winTitle = ['ubah', 'view'].includes(MODE) ? `Location Master - ${options.id}` : 'New Location Master';
		...
		{type: "block", offsetTop:0,offsetLeft:20,blockOffset: 0,list:[
			...
			{type: 'input', name: 'kodesat', label: 'Location Code : ', maxLength: 2,inputWidth: 100,required: true,maxlength:3},
