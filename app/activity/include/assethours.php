...
function save(){	
	...
	if($stat=='ubah'){
		$sql_u = "UPDATE tbl_hours_asset SET amm_code = '{$amm_code}', tanggal = '{$tanggal}', jam = {$jam}, user_modify = '{$user}', date_modify = '{$hari_ini}' WHERE amm_code = '{$amm_code_lm}' AND tanggal = '{$tanggal_lm}'";
	} else {
		$sql_u = "INSERT INTO tbl_hours_asset (amm_code, tanggal, jam, user_create, date_create) VALUES ('{$amm_code}', '{$tanggal}', {$jam}, '{$user}', '{$hari_ini}');";
	}
...
function delete(){	
	...
	$amm_code = $kd[0];
	$tanggal = $kd[1];
	$sql_u = "DELETE FROM tbl_hours_asset WHERE amm_code = '{$amm_code}' AND tanggal = '{$tanggal}'";
	$res = pg_query($conn, $sql_u);
	if($res){
		$ret = "OK";
	}else{
