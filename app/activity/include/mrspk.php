...
function view() {
	...
	$sql = "SELECT a.*, b.wo_date, b.wo_asset, b.wo_desc, b.wo_scheduled, c.amm_desc, b.wo_location ||' - '||d.sl_desc AS asset_location
		...
		WHERE a.spkmr_date >= '{$tglfrom}' AND a.spkmr_date <= '{$tglto}' ORDER BY a.spkmr_code DESC";
	$query=pg_query($conn, $sql);
	while($row=pg_fetch_array($query)){
		echo ("<row id='".$row['spkmr_code']."'>");
		print("<cell><![CDATA[".$i."]]></cell>");
		print("<cell><![CDATA[".$row['spkmr_code']."]]></cell>");
		print("<cell><![CDATA[".$row['spkmr_date']."]]></cell>");
		...
		print("<cell><![CDATA[".$row['spkmr_desc']."]]></cell>");
