  ...
  <script>
	...
	function doOnLoad() {
		const rootLayout = new dhtmlXLayoutObject({
			...
			pattern: '2E',
			cells: [
				{id: "a", text: "Asset Last KM", header: true,height:80},
		...
		grid_reff = rootLayout.cells("b").attachGrid();
       	grid_reff.setHeader("No.,Asset#,Asset Name,Date,Last KM");
        grid_reff.setColumnIds('no,amm_code,amm_desc,tanggal,km');
	...
	function openRequestDetails(windows, options) {
		...
		const winTitle = ['ubah', 'view'].includes(MODE) ? `Last KM : ${options.id}` : 'Last KM Baru';  
	    win.centerOnScreen();
		...
		var forminput = [
			...
			{type:"input", label: "Last KM :", name: "km", inputWidth: 100},
