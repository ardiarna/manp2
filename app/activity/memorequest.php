  ...
  <script>
	...
	function doOnLoad() {
		const rootLayout = new dhtmlXLayoutObject({
			...
			pattern: '2E',
			cells: [
				{id: "a", text: "Memo Request List", header: true,height:80},
	 ...
	 //    						if(resp.xmlDoc.responseText == "OK"){
		...
		grid_reff = rootLayout.cells("b").attachGrid();
       	grid_reff.setHeader("No.,MR#,MR Date,WO#,WO Date,Asset#,Asset Name,Asset Location,Work Description,WO Scheduled");
	...
	function openRequestDetails(windows, options) {
		...
		const winTitle = ['ubah', 'view'].includes(MODE) ? `Memo Request : ${options.id}` : 'Memo Request Baru';  
	    win.centerOnScreen();
