...
function doOnLoad() {
		...
		const rootLayout = new dhtmlXLayoutObject({
				...
				pattern: '2E',
				cells: [
					{id: "a", text: "Retur Penerimaan Barang", header: true,height:80},
		...
		const toolbar = rootLayout.cells("b").attachToolbar(
		 {
			 iconset: 'awesome',
			 xml:"../../libs/toolbar.php?id=stk202"
		});
        ...
        grid_reff.setHeader("No.,Tahun,Refferensi,No.Retur,Tanggal,No.Terima,Tanggal,No.PO,Pemasok,No.DO,Tanggal DO,User Buat,Tanggal Buat,User Edit,Tanggal Edit", null,
							[TC,TC,TC,TC,TC,TC,TC,TC,TL,TL,TC,TL,TL,TL,TL]);		
        grid_reff.setColTypes('ron,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt');
        grid_reff.setInitWidths("50,80,80,150,100,150,100,150,250,120,100,120,100,120,100");
        grid_reff.setColAlign("center,center,center,center,center,center,center,center,left,left,center,right,right,right,left,left,left");
        grid_reff.setColSorting("na,int,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str");
		grid_reff.attachHeader(",#select_filter,#select_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#select_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter");
...
function openRequestDetails(windows, options) {
		  ...
		  const winTitle = ['ubah', 'view'].includes(MODE) ? `Retur Penerimaan Barang - ${options.id}` : 'Retur Penerimaan Barang (Tambah Data)';
		  ...
		  win.attachEvent('onClose', window => {
			  ...
			  return true;
				...
				{type: "block",width: 800,blockOffset: 20,list:[
						...
						{type:"newcolumn"},
						{
							type: 'template',
							name: 'reffbr',
							format: () => '<a href="javascript:void(0);" onclick="showReffWindow(windows,entryForm,\'INVRB\',[\'reff\',\'reffdesc\']);"><img border="0" src="../../assets/imgs/ic_search.gif" style="border:1px solid #a4bed4; padding:2px;" ></a>',
							inputWidth: 25
						},
						{type:"newcolumn"},
				...
				{type: "block", inputWidth: "auto", id: "form_cell_a", list:[	
					{type:"calendar", name: "tglretur", button: "calendar_icon",readonly:true, label:"Tanggal Retur : ",calendarPosition: "right",inputWidth: 150,required: true},
					{type:"input",name: "nopo",	label:"No.PO : ",inputWidth: 150,style:"text-transform: uppercase;",readonly:true},
					{type:"calendar", name: "tglpo", button: "calendar_icon",readonly:true,label:"Tanggal PO : ",calendarPosition: "right",inputWidth: 150,readonly:true},
					...
					{type: "block",width: 500,blockOffset: 50,list:[
						{
							...
							label:"Pemasok : ",
							name: 'pobr',
							format: () => '<a href="javascript:void(0);" onclick="showSupplierWindow(windows,entryForm,[\'vendcode\',\'vendname\']);"><img border="0" src="../../assets/imgs/ic_search.gif" style="border:1px solid #a4bed4; padding:2px;" ></a>',
							inputWidth: 25
						},
						{type:"newcolumn"},
						{type:"input",name: "vendname",offsetLeft:0,inputWidth: 200,required: true,readonly:true,style:"text-transform: uppercase;"},
						{type:"newcolumn"},
						{type:"input", name: "vendcode",inputWidth: 100,required: true,readonly:true},
					...
					{type: "block",width: 500,blockOffset: 50,list:[
						{
							...
							label:"No.Penerimaan : ",
							name: 'pobr',
							format: () => '<a href="javascript:void(0);" onclick="showPurchaseReceipt();"><img border="0" src="../../assets/imgs/ic_search.gif" style="border:1px solid #a4bed4; padding:2px;" ></a>',
							inputWidth: 25
						},
						{type:"newcolumn"},
						{type:"input", name: "noterima", inputWidth: 200,required: true,readonly:true},
						{type:"newcolumn"},
						{type:"input",name: "tglterima",offsetLeft:0,inputWidth: 100,readonly:true,style:"text-transform: uppercase;",readonly:true},
					]},
			...
			myCalendar = new dhtmlXCalendarObject(entryForm.getInput("tglretur"));
			myCalendar1 = new dhtmlXCalendarObject(entryForm.getInput("tglpo"));
	...
	myGrid2.setHeader("Kode Barang,Nama,Satuan,Qty Stock,Qty Terima,Qty Retur,Gudang,Lokasi",null,
...
function showPurchaseReceipt(){
	var kdvendor=entryForm.getItemValue("vendcode");
	if(!kdvendor){
			dhtmlx.alert({
				title: compname,
				type:"alert-warning",
				text:"Pilih Pemasok terlebih dahulu..",
			});
	} else {
		showPurchaseReceiptWindow(windows,entryForm,kdvendor,['noterima','tglterima','nopo','tglpo']);
	}	
