...
function doOnLoad() {
		...
		const rootLayout = new dhtmlXLayoutObject({
			  ...
			  pattern: '1C',
			  cells: [{id: "a",header: false,text: 'Satuan Barang'}],
		...
		const toolbar = cell.attachToolbar(
		 {
			 iconset: 'awesome',
			 xml:"../../libs/toolbar.php?id=stk141"
		});
        ...
        grid_reff.setHeader("No.,Kode,Nama Satuan,User Buat,Tanggal Buat,User Edit,Tanggal Edit", null,
        ...
        grid_reff.setInitWidths("50,100,*,100,100,120,120");
...
function openRequestDetails(windows, options) {
		  ...
		  const winTitle = ['ubah', 'view'].includes(MODE) ? `Satuan Barang - ${options.id}` : 'Satuan Barang Baru';
		...
		{type: "block", offsetTop:0,offsetLeft:20,blockOffset: 0,list:[
			...
			{type: 'input', name: 'kodesat', label: 'Kode Satuan : ', inputWidth: 100,required: true},
