...
<head>
  <meta charset="utf-8"/>
  <title>Transfer Barang Keluar</title>
...
function doOnLoad() {
		...
		const rootLayout = new dhtmlXLayoutObject({
				...
				pattern: '2E',
				cells: [
					{id: "a", text: "Transfer Barang Keluar", header: true,height:80},
		...
		const toolbar = rootLayout.cells("b").attachToolbar(
		 {
			 iconset: 'awesome',
			 xml:"../../libs/toolbar.php?id=stk211"
		});
		...
		grid_reff.setHeader("No.,Tahun,Refferensi,No.Transaksi,Tanggal,Cabang,Gudang,Lokasi,Cabang Tujuan,Total Qty,Keterangan,User Buat,Tanggal Buat,User Edit,Tanggal Edit", null,
							[TC,TC,TC,TC,TC,TL,TL,TL,TL,TL,TL,TL,TL,TL,TL,TL]);
//		grid_reff.setHeader("No.,Tahun,Refferensi,No.Transaksi,Tanggal,Asal,#cspan,#cspan,Tujuan,Total Qty,Keterangan,User Buat,Tanggal Buat,User Edit,Tanggal Edit", null,
//							[TC,TC,TC,TC,TC,TL,TL,,,,TL,TL,TL,TL,TL,TL,TL]);	
//		grid_reff.attachHeader('#rspan,#rspan,#rspan,#rspan,#rspan,Cabang,Gudang,Lokasi,Cabang,Gudang,Lokasi,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan');							
//       grid_reff.setHeader("No.,Tahun,Referensi,No.Transaksi,Tanggal,Asal,#cspan,#cspan,Tujuan,#cspan,#cspan,Total Qty,Keterangan,User Buat,Tanggal Buat,User Edit,Tanggal Edit", null,
//							[TC,TC,TC,TL,TL,TC,,,TC,,,TC,,,TL]);		
...
//		grid_reff.attachHeader('#rspan,#rspan,#rspan,#rspan,#rspan,Cabang,Gudang,Lokasi,Cabang,Gudang,Lokasi,#rspan,#rspan,#rspan,#rspan,#rspan,#rspan,');
        ...
        grid_reff.setInitWidths("50,80,80,150,100,150,150,150,150,100,200,100,120,100,120");
        grid_reff.setColAlign("center,center,center,center,center,centercenter,center,center,center,left,left,right,right,right,left,left,left");
        grid_reff.setColSorting("na,int,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str");
		grid_reff.attachHeader(",#select_filter,#select_filter,#text_filter,#text_filter,#select_filter,#text_filter,#text_filter,#select_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter");
...
function openRequestDetails(windows, options) {
		  ...
		  win.attachEvent('onClose', window => {
			if (!['view',].includes(MODE)) {
			  ...
			  return true;
				...
				{type: "block",width: 900,blockOffset: 20,list:[
						{type:"input", name: "reff", label:"Reff : ",inputWidth: 120,required: true,readonly:true},
						{type:"newcolumn"},
						{
							type: 'template',
							name: 'reffbr',
							format: () => '<a href="javascript:void(0);" onclick="showReffWindow(windows,entryForm,\'INVMK\',[\'reff\',\'reffdesc\',\'kodecabang\',\'namacabang\']);"><img border="0" src="../../assets/imgs/ic_search.gif" style="border:1px solid #a4bed4; padding:2px;" ></a>',
							inputWidth: 25
						},
						{type:"newcolumn"},
						{type:"input", name: "nopo1",inputWidth: 150,readonly:true},
						{type:"newcolumn"},
				...
				{type: "block",width: 300,blockOffset: 0,list:[
					...
					*/
						{type:"newcolumn"},
						{type:"input",label:"Cabang : ", name: "namacabang", inputWidth: 150,readonly:true},
						{type:"newcolumn"},
						{type:"input",hidden:true,name: "kodecabang",offsetLeft:0,inputWidth: 100,readonly:true,style:"text-transform: uppercase;",readonly:true},
					]},
					...
					{type:"input",label:"Keterangan : ",name: "note",inputWidth: 200,rows:1},	
					{type:"newcolumn"},	
					...
					{type: "block",width: 500,blockOffset:50,list:[
						...
						{type:"newcolumn"},
						{type:"input",name: "namacabangtujuan", inputWidth: 200,readonly:true},
						{type:"newcolumn"},
						{type:"input",name: "kodecabangtujuan",offsetLeft:0,inputWidth: 100,readonly:true,style:"text-transform: uppercase;",readonly:true},
					]},
		...
		const toolbarDetailConfig = {
			...
			dhxLayout.cells("a").setHeight(165);
	...
	myGrid2.setHeader("Kode Barang,Nama,Satuan,Qty Stock,Qty Transfer",null,
	...
	myGrid2.setInitWidths("120,*,60,100,100");
