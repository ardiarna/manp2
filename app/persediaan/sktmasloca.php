...
function doOnLoad() {
		...
		const rootLayout = new dhtmlXLayoutObject({
			  ...
			  pattern: '1C',
			  cells: [{id: "a",header: false,text: 'Master Lokasi'}],
        ...
        grid_reff.setHeader("No.,Cabang,Gudang,Kode Lokasi,Nama", null,
        ...
        grid_reff.setInitWidths("50,200,200,200,*");
...
function openRequestDetails(windows, options) {
		...
		{type: "block", offsetTop:0,offsetLeft:20,blockOffset: 0,list:[
			...
			{type: "block", offsetTop:0,blockOffset: 0,list:[
				{type: 'input', name: 'gudang', label: 'Gudang : ', inputWidth: 100,readonly:true,required: true},
				{type:"newcolumn"},
				{
					type: 'template',
					name: 'show_gudang',
				  ...
				  {type:"newcolumn"},
				{type: 'input', name: 'namagudang', inputWidth: 200,readonly:true},
			]},			
			{type: 'input', name: 'kodelokasi', label: 'Kode Lokasi : ', maxLength:5,inputWidth: 100,required: true},
...
function showGudang(){
	var kdcabang=entryForm.getItemValue("cabang");
	if(!kdcabang){
			dhtmlx.alert({
				title: compname,
				type:"alert-warning",
		...
		showGudangWindow(windows,entryForm,kdcabang,['gudang','namagudang']);
	}
