;(function (global, factory) {
  ...
  const auth = {
    ...
    /**
     * Get all allowed subplants for the current logged in user.
     * @returns {Promise<T | never>}
     */
    getCurrentPlantInfo() {
      return axios.get(`${BASE_URL}/auth/GetCurrentPlantInfo.php?mode=json`)
        .then(response => {
          const data = response.data.data;

          if (!data.hasOwnProperty('subplants') && !data.hasOwnProperty('plant')) {
            console.error(JSON.stringify(data));
            throw new Error('Malformed response!')
          }
          if (!Array.isArray(data.subplants)) {
            console.error(JSON.stringify(data));
            throw new Error('Malformed response!')
          }
          if (!Number.isInteger(data.plant)) {
            console.error(JSON.stringify(data));
            throw new Error('Malformed response!')
          }
          return data
    ...
    checkAuthorityToEditMasterData(userid) {
      return axios.get(`${BASE_URL}/auth/CheckIsAuthorizedToEditMasterData.php?mode=json&userid=${userid}`)
        .then(response => response.data.data.is_authorized)
        .catch(errorHandler)
    },
  ...
  function getBlockQuantityList(subplant) {
  ...
  function fetchItemsAvailableForBlocking(subplant, quality, size, shading, motif) {
  ...
  function getBlockQuantityRequests(subplant) {
  ...
  function createBlockQuantityRequests(requestedSubplant, customer, orderTargetDate, keterangan, motifIdS, motifNameS, qualityS, sizeS, shadingS, qtyS) {
  ...
  function updateBlockQuantityRequests(orderId, requestedSubplant, customer, orderTargetDate, keterangan, motifIdS, motifNameS, qualityS, sizeS, shadingS, qtyS) {
    const s_orderTargetDate = dateToString(orderTargetDate);
  ...
  const location = {
    ...
    addPalletsToLocation, removePalletsFromLocation, getBlockQuantityList, fetchItemsAvailableForBlocking, getBlockQuantityRequests, getBlockQuantityDetails, createBlockQuantityRequests, updateBlockQuantityRequests, cancelBlockQuantityRequests, completeBlockQuantityRequests
  };
  const stock = {
    getPalletInfo,
