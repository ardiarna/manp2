(function (global, factory) {
  ...
  function fetchItemData(subplant, quality, size, shading, motifName) {
    return WMSApi.location.fetchItemsAvailableForBlocking(subplant, quality, size, shading, motifName)
  }
  ...
  function openAvailableItemsWindow(windows, subplant, parentWin = null) {
    ...
    return new Promise((resolve, reject) => {
      ...
      win.maximize();
      win.setText('Daftar Motif untuk di Blokir Quantity');
      windowId = win.getId();
      ...
      filterForm.attachEvent('onButtonClick', id => {
        if (id === 'getdata') {
          ...
          win.progressOn();
          fetchItemData(subplant, quality, size, shading, motifName)
            .then(result => ({
              data: result.map(pallet => Object.assign(pallet, {
                id: pallet.motif_id+pallet.quality+pallet.size+pallet.shading,
                is_checked: selectedItems.has(pallet.motif_id+pallet.quality+pallet.size+pallet.shading)
              }))
            }))
            .then(data => {
              ...
              data.data.filter(pallet => pallet.is_checked).forEach(pallet => {
                selectedItems.set(pallet.motif_id+pallet.quality+pallet.size+pallet.shading, true);
              });
      ...
      grid.setHeader('&nbsp;,MOTIF ID,MOTIF NAME,QUALITY,SIZE,SHADE,QTY IN WH,BLOCK QTY,AVAILABLE QTY',null,
        ['', '', '', '', '', '', TEXT_RIGHT_ALIGN, TEXT_RIGHT_ALIGN, TEXT_RIGHT_ALIGN]);
      grid.setColumnIds('is_checked,motif_id,motif_name,quality,size,shading,qty_wh,qty_block,qty_ava');
      grid.setColTypes('ch,rotxt,rotxt,rotxt,rotxt,ron,ron,ron,ron');
      grid.setInitWidths('70,120,*,80,50,60,120,120,120');
      grid.setColAlign('center,left,left,center,left,left,right,right,right');
      grid.setNumberFormat('0,000', grid.getColIndexById('qty_wh'), ".", ",");
      grid.setNumberFormat('0,000', grid.getColIndexById('qty_block'), ".", ",");
      grid.setNumberFormat('0,000', grid.getColIndexById('qty_ava'), ".", ",");
      grid.attachHeader(['&nbsp;', FILTERS.TEXT, FILTERS.TEXT, FILTERS.SELECT, FILTERS.SELECT, FILTERS.TEXT, FILTERS.TEXT, FILTERS.TEXT, FILTERS.TEXT]);
