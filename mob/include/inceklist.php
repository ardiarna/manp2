...
function simpan($stat){
	...
	foreach ($r[cd_code] as $i => $cd_code) {
		...
		$sql .= "INSERT INTO tbl_ceklist_detail (asset_code, tanggal, cd_code, cd_value, cd_note) values('{$r[asset_code]}', '{$r[tanggal]}', '{$cd_code}', '{$cd_value}', '{$cd_note}'); ";
	}
...
function cboasset(){
	global $conn;
	$where1 = "";
	if($_SESSION["user"] == 'shift.utility' || $_SESSION["user"] == 'bengkel.utility') {
		$where1 .= " and amm_location = '12' ";
	}
	$sql = "SELECT amm_code, amm_code||' - '||amm_desc AS lbl FROM assets_master_main WHERE amm_status = 'Active' $where1 ORDER BY amm_desc";
	$query = pg_query($conn, $sql);
...
function display_node($ceklist_code, $parent, $padding) {
	...
	while($r=pg_fetch_array($query)) {
		...
		$out .= '<tr><td class="text-center" style="max-width:200px;">'.$no.'</td><td style="padding-left:'.$padding.'px;"><input type="hidden" name="cd_code[]" value="'.$r[cd_code].'">'.$no2.' '.$r[cd_name].'</td><td class="text-center"><label class="radio-inline" style="margin-left: 40px;"><input type="radio" name="cd_value_'.$r[cd_code].'" value="V" onClick="tampilTrNote(\''.$r[cd_code].'\')"> V </label><label class="radio-inline" style="margin-left: 40px;"><input type="radio" name="cd_value_'.$r[cd_code].'" value="X" onClick="tampilTrNote(\''.$r[cd_code].'\')"> X </label></td></tr><tr id="trnote_'.$r[cd_code].'" style="display:none;"><td>&nbsp;</td><td style="padding-left:'.$padding.'px;" class="text-center">Keterangan : <input type="text" class="form-control input-sm" name="cd_note_'.$r[cd_code].'"></td><td class="text-center">Khusus Utility : <input type="text" class="form-control input-sm" name="cd_uty_'.$r[cd_code].'"></td></tr>';
		if ($r[am_count] > 0) {
...
function display_node_nilai($ceklist_code, $parent, $padding, $asset_code, $tanggal) {
	...
	$query = pg_query($conn,$sql);
	while($r = pg_fetch_array($query)) {
		$sql2 = "SELECT cd_value, cd_note FROM tbl_ceklist_detail WHERE asset_code = '{$asset_code}' AND tanggal = '{$tanggal}' AND cd_code = '{$r[cd_code]}';";
		...
		$out .= '<tr><td class="text-center" style="max-width:200px;">'.$no.'</td><td style="padding-left:'.$padding.'px;"><input type="hidden" name="cd_code[]" value="'.$r[cd_code].'">'.$no2.' '.$r[cd_name].'</td><td class="text-center"><label class="radio-inline" style="margin-left: 40px;"><input type="radio" name="cd_value_'.$r[cd_code].'" value="V" '.$checked_v.'> V </label><label class="radio-inline" style="margin-left: 40px;"><input type="radio" name="cd_value_'.$r[cd_code].'" value="X" '.$checked_x.'> X </label></td></tr><tr><td>&nbsp;</td><td style="padding-left:'.$padding.'px;" class="text-center" colspan="2"><input type="text" class="form-control input-sm" name="cd_note_'.$r[cd_code].'" value="'.$r2[cd_note].'"></td></tr>';
		if ($r[am_count] > 0) {
